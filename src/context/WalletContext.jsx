...
export const WalletProvider = ({ children }) => {
    ...
    try {
      if (type === 'freighter') {
        ...
        if (pk) {
          ...
          localStorage.setItem('pactflow_wallet', JSON.stringify({ key: pk, type }));
        } else {
        ...
        if (rabe) {
          try {
            ...
            if (res) {
              ...
              setBalance(realBalance);
              localStorage.setItem('pactflow_wallet', JSON.stringify({ key: res, type }));
            }
          } catch (e) {
  ...
  const disconnectWallet = () => {
    ...
    localStorage.removeItem('pactflow_wallet');
  };

  useEffect(() => {
    const restoreSession = async () => {
      try {
        const saved = localStorage.getItem('pactflow_wallet');
        if (saved) {
  ...
  const callContract = async (contractId, method, args = [], readOnly = false) => {
    try {
      ...
      const effectivePublicKey = publicKey || "GB5H54HNJMNINL4CURQ34YCH3BBUDNV3CMENBIAH4YO6F37S5E2MMAVV"; // Admin key as dummy for simulation
