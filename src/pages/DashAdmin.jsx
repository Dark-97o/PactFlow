...
const DashAdmin = () => {
  ...
  const REGISTRY_CONTRACT_ID = "CDICFUHEFDULYY46R7AUWLX6UYNQAVB2UPMOPE3K5K66IZ75M2QBLHMQ";
  const ESCROW_CONTRACT_ID = "CBLV25B7NI7FUY7IYGVUOSMNN2GH4IXQP2SB5PXNFGGLD2DIAW5DAU6F"; // v2.5 redeployed
  const OLD_ESCROW_CONTRACT_ID = "CDFCTLTSPSIS245OGCHTUWIUCSVUCWGF7CSBHS5FLK227JGUGCMJH4OS"; // v2.4 deprecated
  const PROOFWORK_CONTRACT_ID = "CDITPAUJYQZZCKMPH6GGNWB6AGCITWCNYWX3CJSRNEKMUKXDJPSES23C";
  const NFT_MARKETPLACE_ID = "CCHZEMKRQ7MJ6XZBUR5XTCF6X2G2A56ZSMKQZPTKXDPPH5CXETAPK444";
  ...
  const DEPLOYED_CONTRACTS = [
    ...
    { name: 'Marketplace', id: NFT_MARKETPLACE_ID, status: 'OPERATIONAL' }
  ];
  ...
  const fetchAdminData = async () => {
    try {
      ...
      let nftCounter = '0';
      try {
        const counterRes = await callContract(NFT_MARKETPLACE_ID, 'get_counter', [], true);
        nftCounter = String(counterRes?.result || 0);
      } catch (err) {
        console.warn("Failed to fetch NFT counter:", err);
      }
    ...
    <DashboardLayout role="admin" activeTab={activeTab} onTabChange={setActiveTab}>
              ...
              <div key={job.id} className="luxury-panel" style={{ padding: '0', border: '1px solid rgba(255,68,68,0.2)', background: 'linear-gradient(180deg,rgba(25,5,5,0.95) 0%,rgba(10,10,15,0.98) 100%)' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '0', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                  ...
                  <div style={{ padding: '20px 25px', borderRight: '1px solid rgba(255,255,255,0.05)' }}>
                    ...
                    <div style={{ fontSize: '1rem', fontWeight: 'bold', color: '#fff', marginBottom: '4px' }}>{job.companyName || 'Unknown'}</div>
                  ...
                  <div style={{ padding: '20px 25px', borderRight: '1px solid rgba(255,255,255,0.05)', background: 'rgba(0,0,0,0.15)' }}>
                    ...
                    <div style={{ fontSize: '0.9rem', color: '#fff' }}>{job.deadline || 'N/A'}</div>
                  </div>
                  {/* Worker Evidence */}
                  <div style={{ padding: '20px 25px' }}>
                    ...
                    <div style={{ fontSize: '1rem', fontWeight: 'bold', color: '#fff', marginBottom: '4px' }}>{job.workerAddress?.slice(0, 8)}...{job.workerAddress?.slice(-6)}</div>
                    {job.githubHash ? (
                ...
                <div style={{ padding: '25px', background: 'rgba(0,0,0,0.3)', borderTop: '1px solid rgba(255,255,255,0.04)' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: '30px', alignItems: 'center' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '15px', minWidth: '180px' }}>
                      <button
                        className="btn-luxury"
                        style={{ padding: '12px 20px', fontSize: '0.9rem', letterSpacing: '1px', background: 'linear-gradient(135deg, #ff4444, #cc0000)', color: '#fff', boxShadow: '0 0 20px rgba(255,68,68,0.3)', border: 'none' }}
                        disabled={isProcessing}
      ...
      {activeTab === 'nft_shop' && (
        <div style={{ padding: '20px' }}>
          ...
          {/* Management Section */}
          <hr style={{ border: 'none', borderTop: '1px solid var(--border-ghost)', margin: '60px 0' }} />

          <h2 style={{ fontSize: '1.5rem', textTransform: 'uppercase', marginBottom: '30px', display: 'flex', alignItems: 'center', gap: '15px' }}>
            <Shield size={22} color="var(--primary)" /> NFT Marketplace Management
          </h2>
          <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: '30px' }}>
            <div className="luxury-panel" style={{ padding: '25px' }}>
              <h3 style={{ fontSize: '1rem', color: 'var(--primary)', marginBottom: '20px', letterSpacing: '1px' }}>MINT NEW NFT</h3>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                <div>
                  <label style={{ fontSize: '0.75rem', color: 'var(--text-dim)', display: 'block', marginBottom: '8px' }}>NFT NAME</label>
                  <input type="text" id="nft-name" placeholder="e.g. Overseer Key Elite" className="cyber-input" style={{ width: '100%', padding: '12px' }} />
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                  <div>
                    <label style={{ fontSize: '0.75rem', color: 'var(--text-dim)', display: 'block', marginBottom: '8px' }}>RARITY</label>
                    <select id="nft-rarity" className="cyber-input" style={{ width: '100%', padding: '12px' }}>
                  ...
                  <div>
                    <label style={{ fontSize: '0.75rem', color: 'var(--text-dim)', display: 'block', marginBottom: '8px' }}>PRICE (XLM)</label>
                    <input type="number" id="nft-price" placeholder="100" className="cyber-input" style={{ width: '100%', padding: '12px' }} />
                  </div>
                </div>
                <button
                  className="btn-luxury"
                  ...
                  onClick={async () => {
                    ...
                    setIsProcessing(true);
                    try {
                      const priceStroops = BigInt(Math.floor(parseFloat(price) * 10_000_000));
                      await callContract(NFT_MARKETPLACE_ID, "mint", [
                        name,
                        rarity,
                        { type: 'i128', value: priceStroops }
                      ]);
                      alert(`Successfully minted ${name}!`);
                ...
                >
                  {isProcessing ? 'INITIALIZING MINT...' : 'EXECUTE ON CHAIN MINT'}
                </button>
              </div>
            </div>
            <div className="luxury-panel" style={{ padding: '25px' }}>
              <h3 style={{ fontSize: '1rem', color: 'var(--text-dim)', marginBottom: '20px', letterSpacing: '1px' }}>MARKETPLACE STATUS</h3>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                <div style={{ padding: '15px', background: 'rgba(0,0,0,0.3)', border: '1px solid var(--border-ghost)' }}>
                  <div style={{ fontSize: '0.7rem', color: 'var(--text-dim)', marginBottom: '5px' }}>CONTRACT ID</div>
                  <div style={{ fontSize: '0.8rem', color: 'var(--primary)', wordBreak: 'break-all', fontFamily: 'monospace' }}>{NFT_MARKETPLACE_ID}</div>
                </div>
                <div style={{ padding: '15px', background: 'rgba(0,0,0,0.3)', border: '1px solid var(--border-ghost)' }}>
                  <div style={{ fontSize: '0.7rem', color: 'var(--text-dim)', marginBottom: '5px' }}>TOTAL ASSETS MINTED</div>
                  <div style={{ fontSize: '1.2rem', fontWeight: 'bold' }}>{stats.nftCount}</div>
                </div>
                <button
                  className="btn-outline"
                  onClick={() => window.open(`https://stellar.expert/explorer/testnet/contract/${NFT_MARKETPLACE_ID}`, '_blank')}
                >
                  VIEW_ON_STELLAR_EXPERT
                </button>
              </div>
            </div>
          </div>
                      ...
                      allUsers.map((user, i) => (
                        <tr key={i} style={{ borderBottom: '1px solid rgba(255,255,255,0.03)' }}>
                          <td style={{ padding: '14px' }}>
                            <div style={{ width: '30px', height: '30px', background: '#111', borderRadius: '4px', overflow: 'hidden' }}>
...
// chore: configure eslint rule for unused variables [v4.3.26-2026-05-17]
