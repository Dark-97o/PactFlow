...
const ESCROW_CONTRACT_ID = "CBLV25B7NI7FUY7IYGVUOSMNN2GH4IXQP2SB5PXNFGGLD2DIAW5DAU6F";
  ...
  const renderDashboard = () => (
    <div style={{ padding: '19px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '25px' }}>
        <div>
          <h1 style={{ color: '#fff', fontSize: '2.4rem', fontFamily: "'Satisfy', cursive", margin: 0, background: 'linear-gradient(to right, #fff, var(--primary))', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Hello, {userData?.name || 'Executor'}</h1>
          <div style={{ display: 'flex', alignItems: 'center', gap: '15px', marginTop: '5px' }}>
            ...
            <span style={{ fontSize: '0.9rem', color: '#fff', fontWeight: 'bold', fontFamily: 'monospace' }}>RPT SCORE: {rpt}</span>
          </div>
        </div>
      </div>
        ...
        <div className="cyber-card" style={{ padding: '20px', background: 'linear-gradient(135deg, rgba(0,212,255,0.05) 0%, rgba(0,0,0,0) 100%)', border: '1px solid rgba(0,212,255,0.1)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              ...
              <div style={{ fontSize: '1.4rem', fontWeight: 'bold', color: '#fff' }}>{rpt} <span style={{ fontSize: '0.9rem', color: '#00d4ff' }}>Tokens</span></div>
            </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '20px' }}>
        ...
        <div className="cyber-card" style={{ padding: '19px', background: 'rgba(255,255,255,0.01)' }}>
          ...
          <div style={{ fontSize: '1.1rem', fontWeight: 'bold', color: '#fff', marginTop: '5px' }}>100%</div>
        </div>
      </div>
    ...
    <div style={{ padding: '20px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
        {allJobs.filter(j => j.status === 'OPEN').map((job) => {
          ...
          const companyName = company.name || snapshotName || 'PactFlow Partner';
            ...
            >
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '19px', marginBottom: '18px', padding: '18px', background: 'rgba(255,255,255,0.02)', border: '1px solid var(--border-ghost)' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: '0.95rem', fontWeight: '800', color: '#fff', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{companyName}</div>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  ...
                  <div style={{ fontSize: '1.1rem', fontWeight: 'bold', color: '#fff' }}>{job.budget} <span style={{ fontSize: '0.85rem', color: 'var(--primary)' }}>XLM</span></div>
                </div>
            ...
            <div style={{ padding: '30px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '40px' }}>
              ...
              <div style={{ background: 'rgba(0,0,0,0.25)', padding: '20px', border: '1px solid rgba(255,255,255,0.05)' }}>
                ...
                <form onSubmit={(e) => { e.preventDefault(); const fd = new FormData(e.target); handleSubmitWork(job.id, { hash: fd.get('hash'), doc: fd.get('doc'), note: fd.get('note') }); }} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                  <input name="hash" placeholder="GitHub Commit Hash" defaultValue={job.githubHash} style={{ width: '100%', background: '#111', border: '1px solid var(--border-ghost)', padding: '14px', color: '#fff', fontSize: '0.9rem' }} />
                  <input name="doc" placeholder="Documentation Link" defaultValue={job.docLink} style={{ width: '100%', background: '#111', border: '1px solid var(--border-ghost)', padding: '14px', color: '#fff', fontSize: '0.9rem' }} />
                  <textarea name="note" placeholder="Executor Notes" defaultValue={job.workerNote} style={{ width: '100%', background: '#111', border: '1px solid var(--border-ghost)', padding: '14px', color: '#fff', fontSize: '0.9rem', minHeight: '60px' }} />
        ...
        <form onSubmit={handleUpdateProfile} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <div style={{ padding: '19px', background: 'rgba(255,68,68,0.03)', border: '1px solid rgba(255,68,68,0.1)', borderRadius: '8px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: '15px' }}>
              <input type="password" placeholder="New Access Key" style={{ width: '100%', background: '#0a0a0a', border: '1px solid var(--border-ghost)', padding: '12px', color: '#fff', fontSize: '0.9rem' }} />
  ...
  return (
    <DashboardLayout role="worker" activeTab={activeTab} onTabChange={setActiveTab}>
      <div style={{ marginTop: '20px' }}>
        ...
        {activeTab === 'nft_shop' && <NFTShop role="worker" />}
