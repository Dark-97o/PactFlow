# 🚀 PactFlow: The Dystopian Labor Protocol

**An Exhaustive Technical Manual for the Decentralized Job-Matching, Reputation, and Escrow Ecosystem on the Stellar Network.**

PactFlow bridges the gap between companies and workers with a premium, dystopian "Vault-Tec" interface. Featuring high-fidelity NFT assets, real-time trust scoring, robust smart contract Escrow logic, and an automated payment gateway natively built on the Stellar Testnet (Soroban).

---

# 🔗 Platform Links

- **Live DApp Terminal:** [PactFlow Live Protocol](https://pact-flow-pi.vercel.app/)
- **Payments Gateway Interface:** [Stellar Network Payment Gateway](https://stellar-network-pgt.vercel.app/)
...
1. [Protocol Philosophy & Vision](#1-protocol-philosophy--vision)
2. [Ecosystem Roles & Core Workflows](#2-ecosystem-roles--core-workflows)
   - [The Worker Persona](#the-worker-persona)
   - [The Company Persona](#the-company-persona)
   - [The Administrator Persona](#the-administrator-persona)
...
5. [Smart Contract Infrastructure (Soroban)](#5-smart-contract-infrastructure-soroban)
   - [Registry Contract Deep Dive](#a-registry-contract)
   - [EscrowVault Contract Deep Dive](#b-escrowvault-contract)
   - [ProofWork Contract Deep Dive](#c-proofwork-contract)
   - [NFTMarketplace Contract Deep Dive](#d-nftmarketplace-contract)
6. [Inter-Contract Communication (ICC) Specifications](#6-inter-contract-communication-icc-specifications)
7. [Database Architecture (Firebase / Firestore)](#7-database-architecture-firebase--firestore)
8. [React Frontend Component Architecture](#8-react-frontend-component-architecture)
9. [Revenue Model & Protocol Tokenomics](#9-revenue-model--protocol-tokenomics)
10. [Trust Tiers, RPT & The Redeem Shop](#10-trust-tiers-rpt--the-redeem-shop)
11. [The Genesis NFT Marketplace](#11-the-genesis-nft-marketplace)
12. [Conflict Resolution & Dispute Logic](#12-conflict-resolution--dispute-logic)
13. [Security Protocols & Admin Diagnostics](#13-security-protocols--admin-diagnostics)
14. [Tech Stack Breakdown](#14-tech-stack-breakdown)
15. [User Feedback & Implementation Logs](#15-user-feedback--implementation-logs)
16. [Local Environment Setup Guide](#16-local-environment-setup-guide)
17. [Roadmap & Future Enhancements](#17-roadmap--future-enhancements)
18. [On-Chain vs Off-Chain Execution Paradigm](#18-on-chain-vs-off-chain-execution-paradigm)

---

## 1. Protocol Philosophy & Vision

PactFlow is designed as a **trustless enforcement protocol**. In traditional freelance marketplaces (Upwork, Fiverr), a centralized intermediary holds the funds, arbitrates disputes opaquely, and extracts exorbitant fees (15-20%). 

PactFlow eliminates the middleman. By leveraging **Soroban Smart Contracts** on the Stellar network, funds are locked automatically, transfers are mediated strictly by cryptographic code, and non-transferable Reputation Tokens (RPT) are issued automatically upon success. 
...
PactFlow operates entirely without traditional passwords. Identity is derived cryptographically from Stellar wallets.
...
Compiled to WebAssembly (WASM) and deployed on the Stellar Testnet, these contracts form the immutable backbone of PactFlow.
...
### D. NFTMarketplace Contract
**Contract ID:** `CCHZEMKRQ7MJ6XZBUR5XTCF6X2G2A56ZSMKQZPTKXDPPH5CXETAPK444`
- **Asset Handling:** Manages a digital ledger of NFTs mapped to owners.
- **Royalty Engine:** Automatically intercepts 10% of the XLM value during any `buy()` execution, redirecting it to the protocol treasury before passing the 90% remainder to the seller.

---

## 6. Inter-Contract Communication (ICC) Specifications

Soroban allows smart contracts to invoke each other. PactFlow leverages this for atomic reputation issuance.
...
PactFlow is a self-sustaining protocol generating yield through autonomous smart contract taxation.
...
2. **The Marketplace Royalty (10%):** A perpetual royalty on all primary and secondary NFT sales.
3. **Staking Treasury TVL:** Registration stakes (1K/10K XLM) locked in the Registry contract represent TVL (Total Value Locked), establishing network credibility and filtering out low-effort sybil accounts.
...
## 11. The Genesis NFT Marketplace

A high-fidelity digital collectible marketplace for the Dystopian Labor Protocol.

**Mechanism:** 
- Admin executes `mint()` on the smart contract.
- Users execute `buy()` via the React frontend. The Stellar SDK constructs a transaction that atomically transfers XLM to the seller (and treasury) and assigns the NFT ID to the buyer's public key.
- Owners can call `relist()` to put the asset back on the market, allowing dynamic secondary trading.

**Rarity Asset Mapping:**
- **COMMON/UNCOMMON:** Base level entry.
- **RARE (15K+ XLM):** Deep Cobalt Blue glow. Highly sought after.
- **EPIC (25K+ XLM):** Saturated Purple CRT effects. Rare structural variants.
- **ELITE (50K+ XLM):** Electric Cyan & Silver theme. 
- **LEGENDARY (80K+ XLM):** Red/Gold premium glow.
- **MYTHIC (100K+ XLM Equivalent):** Spectrum-glow animations with high-frequency color shifts.

---

## 12. Conflict Resolution & Dispute Logic

PactFlow implements an impartial, mathematically precise Dispute mechanism to handle contract breaches.
...
## 15. User Feedback & Implementation Logs

### 🔗 [Feedback Form](https://forms.gle/9igRqatABYEQaaoz9)

### 🔗 [Responses Feedback Form](https://docs.google.com/spreadsheets/d/1UKptTg9DPm0wDSm_xh-Ta7Ttkwx7DGkAIUntTCbc-d8/edit?usp=sharing)

### Table 1: Registered Protocol Users
| User Name | Contact (Email) | User Wallet Address |
|---|---|---|
| Abhishek Kumar | `abhishekkumar086038@gmail.com` | `GCO527YCC6DNDK3K6FN654WXAINDGNB35FUFAN3LURDENIIBD7ZFAJN6` |
| Mausikta Kheto | `mausiktakheto@gmail.com` | `GA5HD4OHKJC3G256U5P7A7SV77YAWXFPL4VQBEDXBNVZLUGGJ4D3BLTQ` |
...
| User Name | Contact | User Wallet Address | User Feedback | Commit ID |
|---|---|---|---|---|
| Abhishek Kumar | `abhishekkumar...` | `GCO52...JN6` | Connected with a different account but it didn't show error. | `7f85d1f` |
| Mausikta Kheto | `mausiktakheto...` | `GA5HD...LTQ` | Redeem RPT and Resolving issues | `06b1954` |
| Dikansha Bindal | `bindaldikansha...` | `GB66X...EEZ` | Reported that social buttons were not clickable. | `11c1ec7` |
| Jayant Vaibhav | `jayantvaibhav...` | `GBQI6...4SR` | Suggested dispute resolution mechanism for contracts. | `06b1954` |
| Abhishek Gupta | `abhishekgupta...` | `GBEA2...E72` | Suggested utility for tokens (Coupons/Redemption). | `c920707` |
| Shashank Rai | `shashankrai...` | `GD5CC...WCU` | Company UI confusion; requested an FAQ page. | `11c1ec7` |
...
1. **Clone the Repository**
   ```bash
   git clone https://github.com/Dark-97o/PactFlow.git
   cd PactFlow
   ```
...
To maintain near-zero gas fees while ensuring absolute financial security, PactFlow employs a strict hybrid architecture. Understanding what lives on the Stellar blockchain versus what lives in the Firebase database is critical for developers.
...
https://github.com/user-attachments/assets/007b3743-f94d-4f3b-93af-a80c63aa7e1b

### Mutli Wallet Login

<img width="1862" height="951" alt="image" src="https://github.com/user-attachments/assets/3bf7d9f6-1bff-42d2-9a6c-10b8220a08a7" />

### Landing Page

<img width="1856" height="953" alt="image" src="https://github.com/user-attachments/assets/1288e65f-f07d-4197-a5e8-3c09f060e34f" />
<br>
<img width="1864" height="959" alt="image" src="https://github.com/user-attachments/assets/465cae45-6f2f-4112-bda9-f2cce8caf60b" />

### Login/SignUP

<img width="1194" height="749" alt="image" src="https://github.com/user-attachments/assets/82b0b372-1483-41b2-bc92-de9c2feb3287" />
<br>
<img width="1212" height="944" alt="image" src="https://github.com/user-attachments/assets/a219a781-c599-4a6a-9893-6f4fa121fa09" />

### User Dashbaord

<img width="1858" height="944" alt="image" src="https://github.com/user-attachments/assets/ee4fc50b-1438-4250-8322-8de590938915" />
<br>
<img width="1866" height="951" alt="image" src="https://github.com/user-attachments/assets/f8f377f8-9cdd-41d2-8014-e3c7011906bd" />

### Company Dashboard

<img width="1862" height="953" alt="image" src="https://github.com/user-attachments/assets/8fcf7920-ae49-446f-a514-88ce2bf6a9c8" />
<br>
<img width="1869" height="955" alt="image" src="https://github.com/user-attachments/assets/1e5c8a9d-d632-47e4-8871-913c269d2bf8" />

### Admin Dashboard

<img width="1852" height="950" alt="image" src="https://github.com/user-attachments/assets/a2919f7b-84d4-479a-a260-71df63673c87" />
<br>
<img width="1860" height="949" alt="image" src="https://github.com/user-attachments/assets/d4b90ca6-012f-4676-b4d6-d7bbea5dc431" />

### Redeem Shop

<img width="1860" height="951" alt="image" src="https://github.com/user-attachments/assets/75093ad6-590a-4b51-ba69-a3dfda449d59" />

### NFT Shop

<img width="1863" height="952" alt="image" src="https://github.com/user-attachments/assets/962f46f1-7797-4385-b753-cef8544005dd" />

### FAQ 

Admin
<img width="1557" height="740" alt="image" src="https://github.com/user-attachments/assets/44cf0fe0-0830-4adc-a63c-8d087df2b654" />
Company
<img width="1551" height="770" alt="image" src="https://github.com/user-attachments/assets/981e0db6-98b8-4011-bdb2-c54c0de37e12" />
Worker
<img width="1566" height="763" alt="image" src="https://github.com/user-attachments/assets/315e8b6f-3035-43d3-99f0-1a3bcbaa30f8" />


## Support & Governance

PactFlow is an open protocol designed for the community. If you encounter any bugs, discover security vulnerabilities, or have suggestions, please submit your findings via our official uplink:


<br/>

<p align="center">
  <img width="2202" height="363" alt="pactflow" src="https://github.com/user-attachments/assets/2de07bd0-c5bf-4a65-9b10-446f564fe30c" /><br>
